<?php
/**
 * This file is part of the Techworker\Uuid package.
 *
 * (c) Benjamin Ansbach <benjaminansbach@googlemail.com>
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */
namespace Techworker\Uuid\Exposer;

use Techworker\Uuid;

/**
 * This interface defines the api for an UUID exposer. An exposer tries to read
 * information from an UUID and give it back to you.
 *
 * @package    Techworker\Uuid
 * @subpackage Exposer
 * @author     Benjamin Ansbach <benjaminansbach@googlemail.com>
 * @copyright  2014 Benjamin Ansbach <benjaminansbach@googlemail.com>
 * @license    MIT
 * @link       http://www.techworker.de/
 * @see        http://www.ietf.org/rfc/rfc4122.txt
 */
interface ExposerInterface
{
    /**
     * Gets the unique key for the exposer to identify the data.
     *
     * @return string
     */
    public function key();

    /**
     * Gets an array of exposed data.
     *
     * @return array
     */
    public function data();
}